...
statusDir = "/var/www/html/SeaQuestDAQStatus" #where to write output html stuff - for normal operation
...
class DiskSpace(StatusChecker):
  ...
  def CheckStatus(self):
    ...
    if not disk:
      ...
      self.data.append( StatusDatum( "Available on %s:%s" % (hostname,diskname), "%.1f G" % available, warning=isWarning, problem=isProblem ) )
